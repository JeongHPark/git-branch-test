...
import { validateRefreshToken, removeRefreshToken, saveRefreshToken } from './tokenStore';  // 🔥 올바른 import
...
export const refreshAccessToken = (req: Request, res: Response) => {
  ...
  try {
    ...
    // 4. 새 토큰들 발급 (기존 코드 + 개선)
    const newAccessToken = jwt.sign({ userId }, ACCESS_SECRET, { expiresIn: '1m' });
